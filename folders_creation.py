...
main_path = user_home+'/Cosmostat/Codes/BlendHunter'
...
for i in [3,7,10,12,16,20,22,24,28,30,32,37,42,44]:
    bh_path =  main_path+'/bh_{}'.format(i)
...
for i in [3,7,10,12,16,20,22,24,28,30,32,37,42,44]:
    bh_path =  main_path+'/bh_pad{}'.format(i)
...
"""Generate the folder regrouping the pre-trained weights"""
folder =  main_path+'/pretrained_weights'.format(i)
folder_pad =  main_path+'/pretrained_weights_pad'.format(i)

if os.path.isdir(folder):
    raise FileExistsError('{} already exists. Please remove this '
                      'directory or choose a new path.'
                      ''.format(folder))
if os.path.isdir(folder_pad):
    raise FileExistsError('{} already exists. Please remove this '
                      'directory or choose a new path.'
                      ''.format(folder_pad))
os.mkdir(folder)
...
"""Generate folders for real data (Cosmos images)"""
real =  main_path+'/bh_real'.format(i)
real_pad =  main_path+'/bh_real_pad'.format(i)

if os.path.isdir(real):
    raise FileExistsError('{} already exists. Please remove this '
                      'directory or choose a new path.'
                      ''.format(real))
if os.path.isdir(real_pad):
    raise FileExistsError('{} already exists. Please remove this '
                      'directory or choose a new path.'
                      ''.format(real_pad))

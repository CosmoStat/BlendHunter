...
def uni_(data=None):
    ...
    return (len(np.where(data[0:4000] == 16)[0]) +
            len(np.where(data[4000:8000] == 16)[0]))


def sep_(data=None, sep_results=False, dist=distance):
    """Separated objects"""
    if sep_results:
        return len([i for i in np.where(data[0:4000] != 1)[0]
                    if distance[i] > 20.0])
    else:
        return len([i for i in np.where(data[0:4000] != 'blended')[0]
                    if dist[i] > 20.0])
...
sep_mb = [[mb_(data=x[j], sep=True) for j in range(len(x))]
          for x in sep_results]

bh_fp = [[fp_(data=x[j]) for j in range(len(x))] for x in bh_results]
sep_fp = [[fp_(data=x[j], sep=True) for j in range(len(x))]
          for x in sep_results]
...
# SExtractor
sep_sep = [[sep_(data=flags[i], sep_results=True) for i in range(len(flags))]
           for flags in sep_results]
cls_sep = [[cls_(data=flags[i], sep_results=True) for i in range(len(flags))]
           for flags in sep_results]
ovlps_sep = [[ovlps_(data=flags[i], sep_results=True)
              for i in range(len(flags))] for flags in sep_results]

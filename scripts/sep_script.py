...
class Run_Sep:
    ...
    def _check_dist(self, dists):
        ...
        return np.prod([1 if dist < self.BIG_DISTANCE else 0
                        for dist in dists],
    ...
    def process(self, f, epoch=0):
        ...
        for obj in f:
            ...
            if len(res) == 0:
                ...
                dists_tmp = self.get_dist(res, obj_pos)
                flag = self.check_blend(res)
                n_good_dist = np.sum([1 if self._check_dist([dists_tmp[i]])
                                      else 0 for i in range(true_nobj)],
                                     dtype=int)
